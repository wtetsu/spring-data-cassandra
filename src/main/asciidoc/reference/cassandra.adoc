...
The use of prepared statements can be enabled directly on `CassandraTemplate` (and its asynchronous and reactive variants) by calling `setUsePreparedStatements(true)`.
